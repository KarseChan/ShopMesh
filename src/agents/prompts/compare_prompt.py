"""Compare Agent Prompt — structured comparison with conclusion.

Used for: "国家地理和小米双肩包哪个好", "这两个衬衫怎么选", "帮我对比一下A和B"
Key characteristic: structured comparison + clear recommendation.
"""
...
_SYSTEM_TEMPLATE = """你是一个对比顾问 Agent。你的目标是对多个商品做结构化比较，并给出明确的选择建议。
...
- 每次只调用一个工具
- 不要重复调用已调用过的工具（相同参数）
- 对比必须基于工具返回的真实数据，严禁编造
- 结论要明确，不要说"各有优势"就结束，要给出具体建议
...
def build_system_prompt(state: dict, tool_names: list[str]) -> str:
    """Build the Compare Agent system prompt."""
