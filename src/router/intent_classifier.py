...
- user_goals: What the user wants (list, 1-2 items for multi-label)
...
Usage:
    ...
    # intent_dict = {"user_goals": ["find_product"], "task_type": "product_search", "execution_hint": "direct_search"}
"""
...
async def classify_intent(query: str) -> tuple[dict, float, str]:
    ...
    intent_dict always contains "user_goals" as a list (multi-label).
    ...
    if intent_dict and confidence >= threshold:
        logger.info("intent_resolved",
                     user_goals=intent_dict.get("user_goals", []),
