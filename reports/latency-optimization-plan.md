...
- ⬜ **P-4**(意图/实体提速)—— **现在的最大单点杠杆**:每次 LLM 固定 ~15s 且换不了模型,砍掉 preprocess 这一整次 LLM(常见 query 走规则)约省 14-20s。
- ⬜ **P-2**(收敛 agent 冗余工具)—— 减少 agent 迭代次数;同时修 Q1 展示重复。
- ⬜ **P-6 / P-6'**(离线富化 / 并行)—— 架构级,按需。
